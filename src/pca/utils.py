...
def generate_plots_and_gff(tmp_dir,
                           hmmsearch_dir,
                           trna_dir ,meta_dir,
                           gff_dir,
                           **kwargs):    
    ...
    if not os.path.isfile(checkpoint_file):
        ...
        results_with_annotate = search_results.merge(phrogs_anno, how='inner',
                                                     left_on='tname',
                                                     right_on='phrog')
        ...
        with open(gff_out_dir, "w") as out_handle:
            ...
            for i in GFF.parse(gff_dir):
                ...
                # turns off the labels of cds' without annotations
                ...
                ax, _ = graphic_record.plot(ax=ax1,
                                            strand_in_label_threshold=7,
                                            annotate_inline=False,
                                            figure_height=3)
                ax.set_title(i.id)
                out_name = os.path.join(plots_dir, f"{i.id}.pdf")
